...
export default function ControlPanel() {
  ...
  const runScenario = async (slug: string) => {
    ...
    try {
      ...
      if (!json.ok) {
        toast.error('Scenario failed', { description: json.error?.message });
      }
    } catch (err) {
      toast.error('Scenario failed', { description: (err as Error)?.message ?? 'Network error' });
    } finally {
      setBusy(null);
    }
  };
  ...
  return (
    <div className="flex flex-col p-5 gap-6">
      <Section
        title="Pre-canned scenarios"
        subtitle="Run a complete sequence as a single batch."
      >
        <div className="flex flex-col gap-2">
          {SCENARIOS.map((s) => (
            <button
              ...
              disabled={busy !== null}
              className="text-left px-3 py-2 rounded-md border hover:bg-[color:var(--color-muted)]/60 disabled:opacity-50 transition-colors"
            >
              <div className="flex items-center gap-2">
                <Play className="size-3.5 text-[color:var(--color-muted-foreground)]" />
                <span className="text-sm font-medium">{s.title}</span>
              </div>
              <p className="text-xs text-[color:var(--color-muted-foreground)] mt-0.5">{s.blurb}</p>
            </button>
          ))}
        </div>
      </Section>
      ...
      >
        <QuickActions disabled={busy !== null} onBusy={setBusy} />
      </Section>
